<?php
// --- SERVER-SIDE SEO LOGIC ---
$queryString = $_SERVER['QUERY_STRING'] ?? '';
$albumName = preg_replace('/(\/info|&.*)$/', '', $queryString);
$albumName = str_replace('.json', '', $albumName);

$meta = [
    'title' => 'Photo Album',
    'description' => 'Dynamic photo album viewer',
    'image' => '', 
...
if (!empty($albumName)) {
    $apiUrl = "https://mediamaze.com/json/?" . $albumName;
    ...
    if ($jsonData) {
        ...
        if ($data) {
            $m = $data['meta'] ?? [];
            $meta['title'] = $m['og_title'] ?? $m['twitter_title'] ?? $data['albumTitle'] ?? $meta['title'];
            $meta['description'] = $m['og_description'] ?? $m['twitter_description'] ?? $m['note'] ?? $meta['description'];
            $img = $m['og_image'] ?? $m['twitter_image'] ?? '';
            if (empty($img) && !empty($data['photos']) && isset($data['photos'][0]['url'])) {
                $img = $data['photos'][0]['url'];
            }
            $meta['image'] = $img;
            $meta['card'] = $m['twitter_card'] ?? $meta['card'];
        }
    }
}
?>
...
<head>
    ...
    <meta name="twitter:card" content="<?php echo htmlspecialchars($meta['card']); ?>">
    <meta name="twitter:url" content="<?php echo htmlspecialchars($meta['url']); ?>">
...
<body>
    ...
    <div id="main-viewer">
        ...
        <div id="hud-bar" class="hud-bar">
            ...
            <div class="hud-right">
                <button id="notes-btn" title="Notes (N)" style="display:none;">📝</button>
                <button id="grid-btn" title="Thumbnails (P)">▦</button>
                <button id="music-btn" title="Music (M)">🎵</button>
    ...
    <div id="grid-modal" class="overlay-modal side-panel" style="display: none;">
        <div class="modal-header">
            ...
            <div class="header-controls">
                ...
                <button class="close-overlay-btn panel-close-btn" id="close-grid-btn">✕</button>
            </div>
        </div>
    ...
    <div id="help-modal" class="overlay-modal" style="display: none;">
        <div class="overlay-content" style="max-width: 800px;">
            <h2>Help page ( press H )</h2> <div class="help-columns">
                <div class="help-col">
    ...
    <div id="meta-modal" class="overlay-modal" style="display: none;">
        <div class="overlay-content extended-content">
            ...
            <div id="startup-options" style="margin-top: 25px; display:none; gap:15px; justify-content:center;">
                <button id="btn-view-music" class="action-btn" style="background-color: #28a745;">View with Music 🎵</button>
        ...
        <div class="overlay-content extended-content" style="max-width: 600px;">
            ...
            <div class="tab-content" id="tab-filters">
                <div class="form-group">
                    <label>Start Photo ID (#ID):</label>
                    <select id="opt-id">
                        <option value="">-- Start from Beginning --</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Category Filter (#CAT-):</label>
                    <select id="opt-cat">
                        <option value="">-- No Filter --</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Keyword Filter (#KW-):</label>
                    <div class="input-with-counter">
                        <input type="text" id="opt-kw" placeholder="e.g. Mountain">
                        <span id="kw-count" class="badge">0 matches</span>
                    </div>
                </div>
            </div>
