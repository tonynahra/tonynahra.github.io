...
$(document).ready(function () {
    ...
    $('body').on('click', '.modal-close-btn', function() {
        ...
        $('#content-modal').find('.modal-header').removeAttr('style'); 
    });
    ...
    $('body').on('click', '.modal-play-btn', function() {
        if (slideshowInterval) {
            ...
            slideshowInterval = setInterval(function() {
                 if (currentCardIndex < currentCardList.length - 1) { $('.modal-next-btn').click(); } else { stopSlideshow(); }
            }, speed);
        }
    });
    ...
    $('body').on('change', '.slideshow-speed', function() {
        if (slideshowInterval) {
            $('.modal-play-btn').click(); setTimeout(() => { $('.modal-play-btn').click(); }, 100);
        }
    });
        ...
        if (document.fullscreenElement) {
            ...
            if (target && target.requestFullscreen) {
                target.requestFullscreen().then(() => {
                    if(isTutorialMode) { $('.tutorial-fs-toggle').fadeIn(); }
    ...
    $('body').on('click', '.tutorial-fs-toggle', function() {
        ...
        if($iframe.length) {
             try {
                 ...
                 doc.body.classList.toggle('hide-nav');
             } catch(e) {}
        }
    });
    
    document.addEventListener('fullscreenchange', (event) => {
        if (!document.fullscreenElement) { $('.tutorial-fs-toggle').hide(); }
    });
