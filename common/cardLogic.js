...
case 'chess':
            ...
            $.ajax({
                ...
                success: function(pgnFileContent) {
                    ...
                    const styleId = 'chess-style-' + Date.now(); // Unique ID for our CSS block
                    
                    // State Variables
                    let currentFontSize = 26; 
                    let commentsEnabled = false;

                    // 1. INJECT HTML (Including the <style> block)
                    $modalContent.html(`
                    ...
                    // --- THE FIX: Target the EXACT IDs from your HTML ---
                    const updateChessStyles = () => {
                        console.log("Applying Font Size:", currentFontSize);
                        
                        // We target the specific ID generated by the library: e.g., #chess-board-12345Moves
                        const movesId = `#${boardId}Moves`; 
                        
                        const css = `
                            /* 1. MOVES CONTAINER (The White Box Area) */
                            ${movesId} {
                            ...
                            /* 3. MOVE NUMBERS (Custom <move-number> tags) */
                            ${movesId} move-number {
                                font-size: ${currentFontSize}px !important;
                        ...
                        // Inject the CSS into our style tag
                        $(`#${styleId}`).text(css);
                    };
                    ...
                    function renderGame(index) {
                        ...
                        const movesPanelSpace = 340; // Space for moves + gap
                        ...
                        if (typeof PGNV !== 'undefined') {
                            ...
                            // --- OBSERVER (Comments) ---
                            // We now know the exact ID to look for
                            const movesPanelId = boardId + 'Moves';
                            
                            const checkInterval = setInterval(() => {
                                const movesPanel = document.getElementById(movesPanelId);
                                ...
                                if (movesPanel) {
                                    ...
                                    gameObserver = new MutationObserver(() => {
                                        // The library toggles the class "active" on the <move> tag
                                        const activeMove = movesPanel.querySelector('move.active'); 
                                        if (activeMove) {
                                            let commentText = "";
                                            let next = activeMove.nextElementSibling;
                                            
                                            // Logic to find comments (<span> or class .comment)
                                            while(next && next.tagName !== 'MOVE') {
                                                // Check for text content in non-move tags
