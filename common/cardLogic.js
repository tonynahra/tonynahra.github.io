...
window.cardLogic = window.cardLogic || {};
...
window.handleModalKeys = function(e) {
    // Allow passing a simple object {key: 'ArrowLeft'} from iframe
    const key = e.key || e; 
    
    if (!$('#content-modal').is(':visible')) { $(document).off('keydown.modalNav'); return; } 
    if (e.target && $(e.target).is('input, textarea, select')) return;
    
    if (isTutorialMode && (key === "ArrowLeft" || key === "ArrowRight" || key === " ")) { return; }
...
window.loadModalContent = function(index) {
    ...
    if (loadType === 'tutorial' && manifestUrl) {
        ...
        // FIXED TUTORIAL NAV: PURE JS KEY FORWARDING (NO JQUERY TRIGGER)
    ...
    switch (loadType) {
        case 'markdown': $.ajax({ url: loadUrl, type: 'GET', dataType: 'text', success: function(markdownText) { const htmlContent = typeof marked !== 'undefined' ? marked.parse(markdownText) : '<p>Error: Marked.js library not loaded.</p>' + markdownText; $modalContent.html(`<div class="markdown-wrapper"><div class="markdown-body" style="padding: 20px; background: white; max-width: 800px; margin: 0 auto;">${htmlContent}</div></div>`); if (infoHtml) { $modalContent.append(infoHtml); } window.applyInfoState(); }, error: function() { $modalContent.html('<div class="error-message">Could not load Markdown file.</div>'); } }); break;
        case 'chess': window.loadChessGame(loadUrl, $modal, $modalContent); break;
        case 'html': $.ajax({ url: loadUrl, type: 'GET', success: function(data) { $modalContent.html(data); if (infoHtml) { $modalContent.append(infoHtml); } window.applyInfoState(); }, error: function() { $modalContent.html('<div class="error-message">Could not load content.</div>'); } }); break;
        case 'image':
    ...
    // FORCE FOCUS to enable keyboard immediately
    $('#content-modal').focus();
}
...
window.openCardByTitle = function(titleToFind) { 
    if (!titleToFind) return; const decodedTitle = decodeURIComponent(titleToFind).trim().toLowerCase(); let $card = $('#' + titleToFind); 
    if ($card.length === 0) { $card = $('.card-item').filter(function() { const cardId = $(this).attr('id'); if (cardId && cardId.toLowerCase() === decodedTitle) return true; const cardTitle = $(this).find('h3').text().trim().toLowerCase(); const imgAlt = $(this).find('img.card-image').attr('alt') || ''; return cardTitle === decodedTitle || (imgAlt && imgAlt.toLowerCase() === decodedTitle); }); } 
    // FIX DEEP LINK BLANK POPUP: Unhide card before clicking
    if ($card.length) { 
        $card.removeClass('hidden-card-item').show(); 
        $('html, body').animate({ scrollTop: $card.offset().top - 100 }, 500); 
        $card.click(); 
    } else { console.warn('Deep link card not found:', decodedTitle); } 
}
...
window.loadChessGame = function(loadUrl, $modal, $modalContent) { if (loadUrl.includes('github.com') && loadUrl.includes('/blob/')) { loadUrl = loadUrl.replace('github.com', 'raw.githubusercontent.com').replace('/blob/', '/'); } $modal.addClass('chess-mode'); $('body').addClass('chess-mode-active'); $modal.find('.modal-header').hide(); $.ajax({ url: loadUrl, dataType: 'text', success: function(pgnContent) { window.setupChessUI(pgnContent, $modalContent, $modal); }, error: function() { $modal.removeClass('chess-mode'); $('body').removeClass('chess-mode-active'); $modal.find('.modal-header').show(); $modalContent.html('<div class="error-message">Could not load PGN file.</div>'); } }); }
window.setupChessUI = function(pgnFileContent, $modalContent, $modal) {
