...
// Expose globals
window.currentCardList = currentCardList;
...
window.injectModalStyles = injectModalStyles;

/* === HELPER FUNCTIONS === */
function decodeText(text) { 
    if (!text) return ""; 
    try { var $textarea = $('<textarea></textarea>'); $textarea.html(text); return $textarea.val(); } 
    catch (e) { return text; } 
}
...
function updateSocialMeta(title, desc, image) {
    const setMeta = (property, content) => { 
        ...
        if ($tag.length === 0) $tag = $(`meta[name="${property}"]`); 
...
window.updateSocialMeta = updateSocialMeta;

// === CHESS CLEANUP HELPER (CRITICAL FOR FS FIX) ===
function cleanUpChessListeners() {
    // Force remove any namespaced listeners from chess_logic.js
    $(document).off('.chessMode'); 
...
window.cleanUpChessListeners = cleanUpChessListeners;
...
window.handleCardView = handleCardView;
...
function applyInfoState() {
    ...
    if (window.cardGlobalState.infoVisible) $infoBtn.addClass('active'); 
    else $infoBtn.removeClass('active');
...
window.applyInfoState = applyInfoState;

/* === MODAL KEY HANDLER === */
function handleModalKeys(e) {
    if (!$('#content-modal').is(':visible')) { 
        $(document).off('keydown.modalNav'); return; 
    } 
    if ($(e.target).is('input, textarea, select')) return;
    if (isTutorialMode && (e.key === "ArrowLeft" || e.key === "ArrowRight" || e.key === " ")) { return; }
    
    // Ignore Chess Keys (handled by chess library)
    if ($('#content-modal').hasClass('chess-mode')) {
        if (e.key === "ArrowLeft" || e.key === "ArrowRight" || e.key === " " || e.key.toLowerCase() === "i") return;
    }
    ...
    switch (e.key) { 
        ...
        case "f": 
            ...
            break; 
        case "ArrowUp": 
            if(isTutorialMode) { const $iframe = $('#modal-content-area iframe'); try { $iframe[0].contentDocument.body.classList.add('nav-visible'); } catch(e){} } 
            break;
        case "ArrowDown": 
            if(isTutorialMode) { const $iframe = $('#modal-content-area iframe'); try { $iframe[0].contentDocument.body.classList.remove('nav-visible'); } catch(e){} } 
            break;
    }
}
window.handleModalKeys = handleModalKeys;
...
function buildTableModal(jsonUrl) { 
    ...
    loadLibrary('https://unpkg.com/tabulator-tables@5.5.0/dist/js/tabulator.min.js', 'js', 'Tabulator').then(() => { 
        $.getJSON(jsonUrl, function(data) { 
            ...
            if (!tableData) { $modalContent.html('<div class="error-message">Invalid table data format.</div>'); return; } 
            
            const tableId = 'tabulator-table-' + Date.now(); 
            const tableHtml = `<div class="markdown-wrapper" style="padding:20px; background:#fff; display:flex; flex-direction:column; height:100%;"><h2 style="margin-top:0; color:#333;">${data.title || 'Data Table'}</h2><p style="color:#666; margin-bottom:15px;">${data.description || ''}</p><div id="${tableId}" style="flex:1;"></div></div>`; 
    ...
    }).catch(() => $modalContent.html('<div class="error-message">Failed to load Tabulator.</div>')); 
}
window.buildTableModal = buildTableModal;

function buildChartModal(jsonUrl) { 
    const $modalContent = $('#modal-content-area'); 
    $modalContent.html('<div class="content-loader"><div class="spinner"></div></div>'); 
    
    loadLibrary('https://cdn.jsdelivr.net/npm/chart.js', 'js', 'Chart').then(() => { 
        $.getJSON(jsonUrl, function(data) { 
            const chartId = 'chart-canvas-' + Date.now(); 
            $modalContent.html(`<div class="markdown-wrapper" style="padding:20px; background:#fff; display:flex; flex-direction:column; height:100%;"><h2 style="margin-top:0; color:#333;">${data.title || 'Financial Chart'}</h2><div class="chart-container" style="flex:1; position:relative;"><canvas id="${chartId}"></canvas></div></div>`); 
...
window.buildChartModal = buildChartModal;

function openFromTable(type, id) { 
    const $modal = $('#content-modal'); 
    const $modalContent = $('#modal-content-area'); 
    if (type === 'chess') { 
        ...
        if (id.toLowerCase().endsWith('.xml') || id.includes('x-plain')) { 
...
window.openFromTable = openFromTable;
...
function loadChessGame(loadUrl, $modal, $modalContent) { 
    ...
    if (typeof window.startChessGame === 'function') { 
        ...
        $modalContent.html('<div class="content-loader"><div class="spinner"></div></div>'); 
        $.getScript('common/chess_logic.js')
            .done(function() { window.startChessGame(loadUrl, $modal, $modalContent); })
            .fail(function() { $modalContent.html('<div class="error-message">Failed to load chess logic.</div>'); }); 
    } 
}
window.loadChessGame = loadChessGame;
...
window.loadModalTabContent = loadModalTabContent;
...
function loadModalContent(index) {
    ...
    if (loadType === 'table') { $modalFsBtn.hide(); $modalInfoBtn.hide(); currentTableJsonUrl = loadUrl; buildTableModal(loadUrl); return; }
    if (loadType === 'chart') { $modalFsBtn.hide(); $modalInfoBtn.hide(); buildChartModal(jsonUrl || loadUrl); return; }
...
window.populateSmartKeywords = populateSmartKeywords;
...
window.getCardSearchableText = getCardSearchableText;
...
window.filterCardsGeneric = filterCardsGeneric;
...
window.loadPhotoAlbum = loadPhotoAlbum;
...
window.resultsLoop = resultsLoop;
...
window.filterYouTubeCards = filterYouTubeCards;
...
window.openCardByTitle = openCardByTitle;
...
$(document).ready(function () {
    ...
    $('body').off('click', '.modal-close-btn').on('click', '.modal-close-btn', function() { stopSlideshow(); animateModalClose(); currentCardList = []; currentCardIndex = 0; isTutorialMode = false; $(document).off('keydown.modalNav'); $('#content-modal').find('.modal-header').removeAttr('style'); });
    ...
    $('body').off('click', '.modal-play-btn').on('click', '.modal-play-btn', function() { 
        $(this).blur();
        if (slideshowInterval) { stopSlideshow(); } else { $(this).html('&#10074;&#10074; Pause'); const speed = parseInt($('.slideshow-speed').val()) || 5000; if (currentCardIndex < currentCardList.length - 1) $('.modal-next-btn').click(); else currentCardIndex = -1; slideshowInterval = setInterval(function() { if (currentCardIndex < currentCardList.length - 1) { $('.modal-next-btn').click(); } else { stopSlideshow(); } }, speed); } 
    });
    ...
    $('body').off('click', '.modal-fullscreen-btn').on('click', '.modal-fullscreen-btn', function(e) {
        ...
        const target = document.getElementById('modal-content-area'); // Use parent ID as target
    ...
    $('body').off('click', '.modal-prev-btn').on('click', '.modal-prev-btn', function() { $(this).blur(); stopSlideshow(); if (currentCardIndex > 0) loadModalContent(currentCardIndex - 1); });
    $('body').off('click', '.modal-next-btn').on('click', '.modal-next-btn', function() { $(this).blur(); if (currentCardIndex < currentCardList.length - 1) loadModalContent(currentCardIndex + 1); else stopSlideshow(); });
    ...
    // FIX: Wrapped in function reference to avoid undefined errors during binding
    $('body').off('input', '#youtube-search-box').on('input', '#youtube-search-box', function() { filterYouTubeCards(); }); 
    $('body').off('change', '#youtube-keyword-filter').on('change', '#youtube-keyword-filter', function() { filterYouTubeCards(); });
    
    $('body').off('input', '#post-search-box').on('input', '#post-search-box', () => filterCardsGeneric('#posts-card-list', '#post-search-box', '#post-category-filter', '#post-keyword-filter', '#posts-no-results', 10)); 
    $('body').off('change', '#post-category-filter').on('change', '#post-category-filter', () => filterCardsGeneric('#posts-card-list', '#post-search-box', '#post-category-filter', '#post-keyword-filter', '#posts-no-results', 10)); 
    $('body').off('change', '#post-keyword-filter').on('change', '#post-keyword-filter', () => filterCardsGeneric('#posts-card-list', '#post-search-box', '#post-category-filter', '#post-keyword-filter', '#posts-no-results', 10));
    
    $('body').off('input', '#album-search-box').on('input', '#album-search-box', () => filterCardsGeneric('#photo-album-list', '#album-search-box', '#album-category-filter', '#album-keyword-filter', '#album-no-results', 20)); 
    $('body').off('change', '#album-category-filter').on('change', '#album-category-filter', () => filterCardsGeneric('#photo-album-list', '#album-search-box', '#album-category-filter', '#album-keyword-filter', '#album-no-results', 20)); 
    $('body').off('change', '#album-keyword-filter').on('change', '#album-keyword-filter', () => filterCardsGeneric('#photo-album-list', '#album-search-box', '#album-category-filter', '#album-keyword-filter', '#album-no-results', 20));
    
    $('#content-modal').off('click', '.tab-button').on('click', '.tab-button', function() { $(this).siblings().removeClass('active'); $(this).addClass('active'); const htmlUrl = $(this).data('content-url'); loadModalTabContent(htmlUrl, '#research-tab-content-modal'); });
});
