window.startChessGame = function(loadUrl, $modal, $modalContent) {
    ...
    $.ajax({
        ...
        success: function(pgnFileContent) {
            ...
            // --- EVAL HTML GENERATOR (REMOVED GAME BALANCE BAR) ---
            ...
            $('#chess-close-btn').off('click').on('click', function(e) {
                e.preventDefault(); $('.modal-close-btn').first().click();
            });
        ...
        error: function() {
            $modal.removeClass('chess-mode'); $('body').removeClass('chess-mode-active'); $modal.find('.modal-header').show(); $modalContent.html('<div class="error-message">Could not load PGN file.</div>');
        }
    });
};
