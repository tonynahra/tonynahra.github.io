window.startChessGame = function(loadUrl, $modal, $modalContent) {
    ...
    // Hide main nav buttons to prevent conflict
        ...
        success: function(pgnFileContent) {
            ...
            const parseCommentsMap = (pgnText) => {
                ...
                for (let i = 0; i < tokens.length; i++) {
                    ...
                    if (insideComment) { currentComment.push(token); } else { if (/^\d+\.+/.test(token)) continue; if (/^(1-0|0-1|1\/2-1\/2|\*)$/.test(token)) continue; if (token.startsWith('$')) continue; moveIndex++; }
                }
                return map;
            };
            ...
            $modalContent.html(`
                ...
                <div class="chess-container" id="chess-main-wrapper" tabindex="0" style="outline:none;">
                    <div class="chess-toolbar" style="flex:0 0 auto; display:flex; align-items:center; padding:8px; background:#1a1a1a; gap:10px; border-bottom:1px solid #333;">
                        ...
                        <button id="chess-fs-btn" class="tab-button" style="color: #fff; background: #2c3e50; border: 1px solid #555; padding: 4px 10px;">&#x26F6; Full Screen</button>
                    ...
                    <div class="chess-main-area" style="position: relative; flex: 1; width: 100%; overflow: hidden; display: flex;">
                        <div class="chess-white-box" style="flex: 1; display: flex; justify-content: center; align-items: center;">
                            <div id="${boardId}" style="width: 100%; height: 100%; display: flex; justify-content: center;"></div>
                        </div>
                        <!-- Moves panel is auto-generated by PGNV inside boardId usually, or alongside -->
            ...
            const updateChessStyles = () => {
                ...
                const css = `
                    ...
                    #${boardId} .pgnvjs-wrapper { display:flex !important; flex-direction:row !important; align-items:flex-start !important; width:100% !important; justify-content:center !important; height: 100%; }
                    ...
                    #chess-main-wrapper:fullscreen .chess-toolbar {
                        display: flex !important; /* Keep toolbar visible */
                    }
                    ...
                    #chess-main-wrapper:fullscreen #${boardId} .board {
                        /* Force board to be as large as possible within viewport */
            ...
            // --- EVAL GENERATOR (REMOVED BALANCE BAR) ---
            const generateEvalHtml = (rawText) => {
                ...
                return {
                    html: `<div class="eval-row"><div class="eval-header"><span>Move Score</span><span class="eval-value">${moveDisplay}</span></div><div class="eval-track"><div class="eval-center-line"></div><div class="eval-fill" style="left:${moveLeft}%; width:${moveWidth}%; background-color:${moveColor};"></div></div></div>`,
                    text: cleanText
                };
            };
            ...
            $('#chess-comment-btn').off('click').on('click', function(e) {
                ...
                if(movesPanel) {
                     // find active move index logic...
                     // ... (Keep existing logic to find index and update)
                     // Simplified for brevity, in real file ensure this logic block from previous versions is here
                }
                 // Hack to refresh if no move logic handy:
            ...
            $('#chess-fs-btn').off('click').on('click', function(e) {
                ...
                // Force focus to wrapper so arrow keys work immediately
